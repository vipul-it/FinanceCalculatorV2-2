import {
  ...
  const PrePaymentsHistory = () => {
    ...
    const deleteRecord = id => {
      db.transaction(tx => {
        tx.executeSql(
          'DELETE FROM PrePaymentsHistory WHERE id = ?',
          [id],
          (_, result) => {
    ...
    const deleteAllRecords = () => {
      db.transaction(tx => {
        tx.executeSql('DELETE FROM PrePaymentsHistory', [], (_, result) => {
          if (result.rowsAffected > 0) {
    ...
    const fetchData = () => {
      db.transaction(tx => {
        tx.executeSql('SELECT * FROM PrePaymentsHistory', [], (_, {rows}) => {
  ...
  export default PrePaymentsHistory;
