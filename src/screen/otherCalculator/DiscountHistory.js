...
const DiscountHistory = () => {
  ...
  const deleteRecord = id => {
    db.transaction(tx => {
      tx.executeSql(
        'DELETE FROM DiscountCalculator WHERE id = ?',
        [id],
        (_, result) => {
  ...
  const deleteAllRecords = () => {
    db.transaction(tx => {
      tx.executeSql('DELETE FROM DiscountCalculator', [], (_, result) => {
        if (result.rowsAffected > 0) {
  ...
  const fetchData = () => {
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM DiscountCalculator', [], (_, {rows}) => {
                  ...
                  <View className="flex-row  items-center">
                    ...
                    <View>
                      <Text className="text-primaryHeading font-semibold ">
                        &#8377; {item.priceBeforeDiscount} Apply Discount{' '}
                        {item.discountPercent}%
                      </Text>

                      <Text className="text-primaryHeading font-semibold ">
                        After Discount Price &#8377; {item.priceAfterDiscount}
                      </Text>
                      <Text className="text-primaryDark font-semibold">
                        You Save Total &#8377; {item.youSave}
                      </Text>
                    </View>
                  </View>
                  <View>
